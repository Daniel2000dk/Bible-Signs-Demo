...
type TabType = 'text' | 'color' | 'frame' | 'translation'
...
const tabs: { key: TabType; label: string; step: string }[] = [
  ...
  { key: 'translation', label: 'Vertaling',   step: '04' },
]
    ...
    <section id="personaliseer" className="py-24 md:py-32 bg-bs-offwhite">
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="flex flex-col gap-3 mb-14">
          ...
          <p className="text-[15px] text-bs-dark max-w-md leading-relaxed">
            Kies je waarheid, je kleur en je lijst. Elke combinatie is uniek — net als jij.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="flex gap-0 border-b border-bs-sand">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-0.5 px-4 py-3 text-[12px] font-medium tracking-wide transition-all duration-200 border-b-2 -mb-px ${
                    activeTab === tab.key
                      ...
                      {textOptions.map((opt, i) => (
                        <button
                          ...
                          className={`flex items-center justify-between px-4 py-3.5 border text-left transition-all duration-150 ${
                            selectedTextIndex === i
                              ? 'border-bs-gold bg-bs-gold/8 text-bs-black'
                        ...
                        >
                          <div>
                            ...
                            <p className="text-[11px] text-bs-dark/70 tracking-widest uppercase mt-0.5">{opt.reference}</p>
                          </div>
                          {selectedTextIndex === i && (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {translationOptions.map((tr) => (
                        <button
                          ...
                          className={`px-5 py-2.5 border text-[13px] font-medium transition-all duration-150 ${
                            selectedTranslation === tr
                              ? 'border-bs-gold bg-bs-gold text-bs-black'
                              : 'border-bs-sand text-bs-dark hover:border-bs-dark/40 hover:text-bs-black'
                          }`}
                        >
                          {tr}
                        </button>
            ...
            <div className="flex flex-col gap-4 pt-4 border-t border-bs-sand">
              <div className="flex items-center justify-between text-[13px]">
                <span className="text-bs-dark">Jouw selectie:</span>
                <span className="font-medium text-bs-black">
                  {selectedText.label} · {selectedColor.name} · {selectedTranslation}
                  {selectedFrame !== 'none' ? ` · ${frameOptions.find(f => f.value === selectedFrame)?.label}` : ''}
                </span>
              </div>
              <Link
                href="#shop"
          ...
          <div className="flex flex-col items-center gap-4 lg:sticky lg:top-28">
            <div
              className="relative flex items-center justify-center w-full py-16 rounded-sm"
              style={{
                backgroundColor:
            ...
            <p className="text-[12px] text-bs-dark/60 text-center tracking-wide">
              Live preview — jouw ontwerp, in real time
            </p>
          </div>
        </div>
      </div>
